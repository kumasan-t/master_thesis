\begin{document}
	...
	Using the curve fit tool from Matlab, it has been possible to evaluate the gamma value of the fitting curve for each snapshot such that $\Gamma = \{\gamma_1, \gamma_2, ... , \gamma_d\}$ where $d$ is the number of snapshots, using Bisquare as robust regression and the Trust-Region algorithm; Figure \ref{graph_28_degree} shows the result over one of the graph of the observation period. Then mean, variance and standard deviation has been evaluated over $\Gamma$.
	...
	A random network following the power law degree distribution has been generated yielding very promising results. From the $\gamma$ value, a new graph has been created with a number of nodes in the order of the sizes seen until now, specifically 886 nodes which is the maximum number of nodes observed so far, using the NetworkX function \textit{powerlaw\_sequence} that generates a sample sequence of desired length from a power law distribution with custom exponent. The generated graph comes without self-loops, but also with nodes without edges and smaller components. After removing these nodes, 745 nodes and 2081 edges are left. One of the peculiarity of scale-free networks is that the clustering coefficient is usually low: the result show indeed a clustering coefficient that follows the same trend of the real network.
	...
	\subsection{Lightning Network as a Time Varying Graph}
	
	The generation process of an equivalent of the Lightning Network presented above lacks of the intrinsic nature of the edges, which are temporary 
